...
from .configuration_vision_vision_dual_encoder import VisionVisionDualEncoderForClassificationConfig
...
min_rul=28.0
max_rul=278.0 #从F:\dachang\ViTST_fzy\dataset\CMAPASS_fzy\process_scripts\nomoulize_outcome.py得到的
...
@dataclass
class VisionVisionClassifierOutput(ModelOutput):
    """
    ...
    ID_image_embeds: torch.FloatTensor = None
...
class VisionVisionDualEncoderModelForClassification(PreTrainedModel):
    config_class = VisionVisionDualEncoderForClassificationConfig
    base_model_prefix = "vision_vision_dual_encoder"
    main_input_name = "ID_pixel_values"  # TODO: 当 main_input_name 被设置为 pixel_values 时，这通常意味着模型的输入是像素值，而不是文本序列。
    supports_gradient_checkpointing = True # TODO: 一个布尔值，指示模型是否支持梯度检查点技术，这是一种节省内存的技术。
    def __init__(
        self,
        config: Optional[VisionVisionDualEncoderForClassificationConfig] = None,
        vision_model: Optional[PreTrainedModel] = None,
        ID_vision_model: Optional[PreTrainedModel] = None,
        prediction_head_dropout: float = None
    ):

        if config is None and (vision_model is None or ID_vision_model is None):
            raise ValueError("Either a configuration or an vision and a text model has to be provided")

        if config is None:
            config = VisionVisionDualEncoderForClassificationConfig.from_vision_vision_configs(vision_model.config, ID_vision_model.config)
        else:
        ...
        if ID_vision_model is None:
            if isinstance(config.vision_config, CLIPVisionConfig):
                ID_vision_model = CLIPVisionModel(config.ID_vision_config)
            else:
                ID_vision_model = AutoModel.from_config(config.ID_vision_config)

        # if text_model is None:
        #     text_model = AutoModel.from_config(config.text_config)
        
        self.vision_model = vision_model
        self.ID_vision_model = ID_vision_model
        
        # make sure that the individual model's config refers to the shared config
        # so that the updates to the config will be synced
        self.vision_model.config = self.config.vision_config
        self.ID_vision_model.config = self.config.ID_vision_config
        # print(self.config.vision_config)

        self.vision_embed_dim = config.vision_config.hidden_size
        self.ID_vision_embed_dim = config.ID_vision_config.hidden_size
        ...
        self.projection = nn.Linear(self.vision_embed_dim+self.ID_vision_embed_dim, self.projection_dim, bias=False)
        # self.activation = nn.Tanh()
        self.activation = nn.ReLU()
        self.projection1 = nn.Linear(self.projection_dim,self.projection_dim//2)
        self.projection2 = nn.Linear(self.projection_dim//2, self.projection_dim // 4)
        self.projection3 = nn.Linear(self.projection_dim // 4, self.projection_dim // 8)
        self.projection4 = nn.Linear(self.projection_dim // 8, self.projection_dim // 16)
        self.classifier = nn.Linear(self.projection_dim // 16, self.num_classes) if self.num_classes > 0 else nn.Identity()
    ...
    def get_ID_image_features(
        ...
        >>> image_features = model.get_image_features(**inputs)
        ```"""
        ID_vision_outputs = self.ID_vision_model(
            pixel_values=pixel_values,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ...
        pooled_output = ID_vision_outputs[1]  # pooled_output

        return pooled_output


    @add_start_docstrings_to_model_forward(VISION_TEXT_DUAL_ENCODER_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=VisionVisionClassifierOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        pixel_values=None,
        ...
        # else:
        #     # 验证/测试阶段使用反归一化的标签
        #     denormalized_labels = self.denormalize_labels(labels)
        ...
        if self.ID_vision_model.config.model_type == "swin":
            ID_image_embeds = ID_vision_outputs[1]
        elif self.ID_vision_model.config.model_type == "vit":
            # image_embeds = vision_outputs[0][:,0,:]
            ID_image_embeds = ID_vision_outputs[1]
        else:
            raise Exception("Only support vit and swin vision model right now.")
        ...
        vl_embeds = torch.concat([image_embeds, ID_image_embeds], dim=-1)
        pooled_output = self.dropout(vl_embeds)
        pooled_output = self.projection(vl_embeds)
        pooled_output = self.activation(pooled_output)
        pooled_output = self.projection1(pooled_output)
        pooled_output = self.activation(pooled_output)
        pooled_output = self.projection2(pooled_output)
        pooled_output = self.activation(pooled_output)
        pooled_output = self.projection3(pooled_output)
        pooled_output = self.activation(pooled_output)
        pooled_output = self.projection4(pooled_output)
        pooled_output = self.activation(pooled_output)
        pooled_output = self.dropout(pooled_output)
        """上面这段代码的作用是将图像特征和 ID 图像特征结合起来，并通过一系列变换生成一个综合的特征表示，这个表示
        可能会被用于后续的分类或其他类型的任务。这种特征融合和变换的方法在多模态学习（如结合视觉和文本信息）中很常见。"""


        # classification logits
        logits = self.classifier(pooled_output) # todo:这里的预测值还是太小了，需要调整一下
            ...
            if self.config.problem_type == "regression":
                ...
                if self.num_classes == 1:
                    if is_training:
                        ...
                        # 假设normalized_labels和normalized_logits是归一化后的数组
                        normalized_labels = labels.squeeze()  # 转换为NumPy数组
                        normalized_logits = logits.squeeze()

                        # 反归一化公式
                        denormalized_labels = min_rul + normalized_labels * (max_rul - min_rul)
                        denormalized_logits = min_rul + normalized_logits * (max_rul - min_rul)
                        # 确保denormalized_labels和denormalized_logits是张量
                        # denormalized_labels_tensor = torch.tensor(denormalized_labels)
                        # denormalized_logits_tensor = torch.tensor(denormalized_logits)
                        loss = loss_fct(denormalized_logits, denormalized_labels)
                    # loss = loss_fct(logits.squeeze(), labels.squeeze())
                else:
        ...
        return VisionVisionClassifierOutput(
            loss=loss,
            logits=logits
        )
    ...
    @classmethod
    def from_vision_vision_pretrained(
        cls,
        vision_model_name_or_path: str = None,
        ID_vision_model_name_or_path: str = None,
        ...
        **kwargs,
        ...
        kwargs_ID_vision = {
            argument[len("ID_vision_"):]: value for argument, value in kwargs.items() if argument.startswith("ID_vision_")
        }
        ...
        for key in kwargs_ID_vision.keys():
            del kwargs["ID_vision_" + key]
        ...
        if vision_model is None:
            ...
            if "config" not in kwargs_vision:
                ...
                # ID_vision_config= AutoConfig.from_pretrained(ID_vision_model_name_or_path)
        ...
        # Load and initialize the vision and text model
        ID_vision_model = kwargs_ID_vision.pop("model", None)
        if ID_vision_model is None:
            if vision_model_name_or_path is None:
                raise ValueError(
                    "If `vision_model` is not defined as an argument, a `vision_model_name_or_path` has to be defined"
            ...
            if "config" not in kwargs_ID_vision:
                # vision_config = AutoConfig.from_pretrained(vision_model_name_or_path)
                ID_vision_config= AutoConfig.from_pretrained(ID_vision_model_name_or_path)


            if ID_vision_config.model_type == "clip":
                kwargs_ID_vision["config"] = ID_vision_config.vision_config
                ID_vision_model = CLIPVisionModel.from_pretrained(ID_vision_model_name_or_path, *model_args, **kwargs_ID_vision)
                # TODO: Should we use the pre-trained projection as well ?
            elif ID_vision_config.model_type == "vit":
                # our modified vit
                ID_vision_model = ViTModel.from_pretrained(ID_vision_model_name_or_path, *model_args, **kwargs_ID_vision)
            elif ID_vision_config.model_type == "swin":
                # our modified swin
                ID_vision_model = SwinModel.from_pretrained(ID_vision_model_name_or_path, *model_args, **kwargs_ID_vision)
            else:
                kwargs_ID_vision["config"] = ID_vision_config
                ID_vision_model = AutoModel.from_pretrained(ID_vision_model_name_or_path, *model_args, **kwargs_ID_vision)
        ...
        config = VisionVisionDualEncoderForClassificationConfig.from_vision_vision_configs(vision_model.config, ID_vision_model.config, num_classes, **kwargs)

        # init model
        model = cls(config=config, vision_model=vision_model, ID_vision_model=ID_vision_model)
