...
SWIN_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    # See all Swin models1 at https://huggingface.co/models?filter=swin
]
...
# Copied from transformers.models1.beit.modeling_beit.drop_path
...
# Copied from transformers.models1.beit.modeling_beit.BeitDropPath with Beit->Swin
...
class SwinSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Normalize the attention scores to probabilities.todo:这里将注意力归一化为概率，我们回归就别管了
...
class SwinForMaskedImageModeling(SwinPreTrainedModel):
    def __init__(self, config):
        ...
        # configurations
        self.config = config
        self.image_size = self.config.image_size if isinstance(self.config.image_size, collections.abc.Iterable) else (self.config.image_size, self.config.image_size)
        self.patch_size = self.config.patch_size if isinstance(self.config.patch_size, collections.abc.Iterable) else (self.config.patch_size, self.config.patch_size)
        self.mask_patch_size = self.config.mask_patch_size if isinstance(self.config.mask_patch_size, collections.abc.Iterable) else (self.config.mask_patch_size, self.config.mask_patch_size)
    ...
    def forward(
        ...
        wh_ratio = self.image_size[1] / self.image_size[0]
        height = math.floor((sequence_length*wh_ratio)**0.5)
        ...
        masked_im_loss = None
        if bool_masked_pos is not None:
            size = (self.image_size[0] // self.patch_size[0], self.image_size[1] // self.patch_size[1])
            bool_masked_pos = bool_masked_pos.reshape(-1, size[0], size[1])
            mask = (
                bool_masked_pos.repeat_interleave(self.patch_size[0], 1)
            ...
            # # punish the model if it only generate while blank
            # penality_loss = (reconstructed_pixel_values * mask).mean()
            # masked_im_loss += penality_loss
    ...
    def unpatchify(self, patchified_pixel_values):
        ...
        num_patches_one_direction = (self.image_size[0] // self.patch_size[0], self.image_size[1] // self.patch_size[1])
        # sanity check
        if num_patches_one_direction[0]*num_patches_one_direction[1] != patchified_pixel_values.shape[1]:
            print(num_patches_one_direction)
            print(patchified_pixel_values.shape[1])
            raise ValueError("Make sure that the number of patches is the multiply of num_patches_one_direction")

        # unpatchify
        batch_size = patchified_pixel_values.shape[0]
        patchified_pixel_values = patchified_pixel_values.reshape(
            batch_size,
            num_patches_one_direction[0],
            num_patches_one_direction[1],
            patch_size[0],
            patch_size[1],
            num_channels,
        )
        ...
        pixel_values = patchified_pixel_values.reshape(
            batch_size,
            num_channels,
            num_patches_one_direction[0] * patch_size[0],
            num_patches_one_direction[1] * patch_size[1],
        )
        return pixel_values
