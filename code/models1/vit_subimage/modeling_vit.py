...
VIT_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    # See all ViT models1 at https://huggingface.co/models?filter=vit
]
...
class ViTEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        # height = width = 64 # local sub image
        nh = height // 64
        nw = width // 64
        h0 = 64 // self.config.patch_size
        w0 = 64 // self.config.patch_size
        ...
        # repeat
        patch_pos_embed = patch_pos_embed.view(1, 4, 4, dim)
        patch_pos_embed = patch_pos_embed.tile((1, nh, nw, 1))
...
class ViTSelfOutput(nn.Module):
    """
    The residual connection is defined in ViTLayer instead of here (as is the case with other models1), due to the
...
class ViTModel(ViTPreTrainedModel):
    ...
    def my_own_get_head_mask(self, head_mask, config):
        def get_room(idx, w):
            room_x = (idx - 1) // (4 * w) // 4
            room_y = (idx - 1) % (4 * w) // 4
            return (room_x, room_y)

        def generate_array(h, w):
            # Initialize a 16hw x 16hw array with zeros
            arr = torch.zeros((16*h*w, 16*h*w), dtype=int)

            # Loop over all the possible pairs of indices
            for i in range(1, 16*h*w + 1):
                for j in range(1, 16*h*w + 1):
                    # Check if the two people are in the same room
                    if get_room(i, w) == get_room(j, w):
                        arr[i-1][j-1] = 1

            # Add a row of all 1s at the beginning
            arr = torch.vstack([torch.ones((1, 16*h*w), dtype=int), arr])

            # Add a column of all 1s at the beginning
            arr = torch.hstack([torch.ones((16*h*w + 1, 1), dtype=int), arr])

            return arr.cuda()

        if head_mask is not None:
            return [head_mask for i in range(config.num_hidden_layers)]
        else:
    ...
    def forward(
        ...
        head_mask = self.my_own_get_head_mask(head_mask, self.config)
