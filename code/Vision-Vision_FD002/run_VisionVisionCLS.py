...
from models.vision_vision_dual_encoder.modeling_vision_vision_dual_encoder import VisionVisionDualEncoderModelForClassification
from models.vision_vision_dual_encoder.configuration_vision_vision_dual_encoder import VisionVisionDualEncoderForClassificationConfig
from draw_hot import draw_CAM
...
def fine_tune_hf(
    ...
    if do_train and not continue_training:
        if image_size and grid_layout and ID_image_size and ('vit' in image_model_path or 'swin' in image_model_path):
            model = VisionVisionDualEncoderModelForClassification.from_vision_vision_pretrained(
            image_model_path,ID_image_model_path , num_classes=num_classes,
            vision_image_size=image_size, vision_grid_layout=grid_layout,
            ID_vision_image_size = ID_image_size
            )
        else:
            model = VisionVisionDualEncoderModelForClassification.from_vision_vision_pretrained(
            image_model_path, ID_image_model_path, num_classes=num_classes
        ...
        if os.path.exists(output_dir):
            ...
            if latest_checkpoint_idx > 0 and os.path.exists(os.path.join(output_dir, f"checkpoint-{latest_checkpoint_idx}")):
                ...
                vision_text_config = VisionVisionDualEncoderForClassificationConfig.from_pretrained(ft_model_path)
                model = VisionVisionDualEncoderModelForClassification.from_pretrained(ft_model_path, config=vision_text_config)
                print("load from the last checkpoint", image_model_path)
            else: # don't have a fine-tuned model
                if image_size and ID_image_size and grid_layout:
                    model = VisionVisionDualEncoderModelForClassification.from_vision_vision_pretrained(
                    image_model_path, ID_image_model_path, num_classes=num_classes, vision_image_size=image_size, vision_grid_layout=grid_layout,ID_vision_image_size = ID_image_size
                    )
                else:
                    model = VisionVisionDualEncoderModelForClassification.from_vision_vision_pretrained(
                    image_model_path, ID_image_model_path, num_classes=num_classes
    ...
    # feature_extractor.do_normalize = False #todo:由于是回归任务，因此不要归一化
    ...
    def preprocess_train(example_batch):
        ...
        example_batch["ID_pixel_values"] = [
            train_transforms(image.convert("RGB")) for image in example_batch["ID_image"]
        ]
    ...
    def preprocess_val(example_batch):
        ...
        example_batch["ID_pixel_values"] = [
            val_transforms(image.convert("RGB")) for image in example_batch["ID_image"]
        ]
    ...
    def collate_fn(examples):
        ...
        return {"pixel_values": pixel_values,"ID_pixel_values": ID_pixel_values, "labels": labels}
    ...
    # ID_train_dataset.set_transform(preprocess_val)
    ...
    # combined_dataset = ConcatDataset([train_dataset, train_dataset])
    # combined_dataset = ConcatDataset([combined_dataset, train_dataset])
    # combined_dataset = ConcatDataset([combined_dataset, train_dataset])
    # combined_dataset = ConcatDataset([combined_dataset, train_dataset])
    # combined_dataset = ConcatDataset([combined_dataset, train_dataset])
    trainer = Trainer(
    ...
    # callbacks=[EarlyStoppingCallback(early_stopping_patience=5)] # todo:这个代码是5轮没有提升就停止训练
    )

    # todo:画出扫描后的热图
    # draw_CAM(model, '../../dataset/CMAPASS_fzy/processed_data/FD002_order_differ_interpolation_-x1_xx2_21x1_1344x1024_split0_cut_by100.0/1.png', '../../dataset/CMAPASS_fzy/processed_data/热力图/1.png', isSave=True,
    #          isShow=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_prefix', type=str, default='FD002_order_differ_interpolation_-x1_xx2_21x1_1344x1024_') #
    parser.add_argument('--dataset_condition', type=str,default='ConID_FD002_order_differ_interpolation_-x1_xx2_21x1_1344x1024_')  #
    ...
    parser.add_argument('--ID_image_model_path', type=str, default='swin', choices=['swin','vit']) #
    ...
    print(f'Dataset used: {dataset}, prefix: {dataset_prefix},dataset_condition: {dataset_condition}.')
    ...
    if dataset == 'FD001':
        ...
        epochs = 20
        image_size = (1344,1024)
        ID_image_size=(512,1024)
        grid_layout = (21,1)
    elif dataset == 'FD002':
        base_path = '../../dataset/CMAPASS_fzy'
        num_classes = 1
    ...
    ID_image_model_path = args.ID_image_model_path
    if ID_image_model_path == "vit": # default vit
        ID_image_model_path = "google/vit-base-patch16-224-in21k"
        patch_size = 16
    elif ID_image_model_path == "vit-384":
        ID_image_model_path = "google/vit-base-patch16-384"
        patch_size = 16
    elif ID_image_model_path == "swin": # default swin
        ID_image_model_path = "microsoft/swin-base-patch4-window7-224-in22k"
        patch_size = 4
    elif ID_image_model_path == "swin-224":
        ID_image_model_path = "microsoft/swin-base-patch4-window7-224"
        patch_size = 4
    elif ID_image_model_path == "resnet":
        ID_image_model_path = "microsoft/resnet-50"
    # max_length = 64
    ...
    for missing_ratio in missing_ratios:
        ...
        # for k in range(4,5):
            ...
            # the path to save models1
            if args.output_dir is None:
                if args.finetune_mim:
                    output_dir = f"../../ckpt/VisionTextCLS/{dataset_prefix}{dataset}_{image_model}{freeze_vision_model}-{ID_image_model_path}{freeze_text_model}_mim_{mask_patch_size}_{mask_ratio}_{mask_method}/split{split_idx}"
                else:
                    output_dir = f"../../ckpt/VisionTextCLS/{dataset_prefix}{dataset}_{image_model}{freeze_vision_model}-{ID_image_model_path}{freeze_text_model}/split{split_idx}"
            else:
                output_dir = f"../../ckpt/VisionTextCLS/{args.output_dir}/split{split_idx}"

            # prepare the data:
            Ptrain, Pval, Ptest, ytrain, yval, ytest = get_data_split(base_path, split_path, split_idx, dataset=dataset, prefix=dataset_prefix, condition=dataset_condition,upsample=upsample, missing_ratio=missing_ratio,max_tmins=cut_nub)
            ...
            for m in range(n_runs):
                ...
                ID_image_model_path=ID_image_model_path,
