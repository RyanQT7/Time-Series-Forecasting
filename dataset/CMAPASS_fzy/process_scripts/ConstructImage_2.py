...
def draw_image(pid, split_idx, ts_orders, ts_values, ts_times, ts_condition, ts_params, ts_scales,
    ...
    # 滑动窗口配置
    window_size = max_tmins
    slide_step = 1
    num_windows = (max(ts_times[0]) - window_size) // slide_step + 1  # 计算滑动窗口数量
    ...
    img_path2 = os.path.join(base_path2, f"{pid}.png")
    # if os.path.exists(img_path):
    # if not override:
    #     return []
    ...
    for window_idx in range(int(num_windows)):
        ...
        start_time = window_idx * slide_step
        end_time = start_time + window_size
        param_scale_x = [start_time, end_time]  # 设置X轴范围为当前窗口

        # 修改保存路径和文件名以区分不同窗口
        img_path = os.path.join(base_path, f"{pid}_{window_idx}.png")
        img_path2 = os.path.join(base_path2, f"{pid}_{window_idx}.png")
        ...
        for idx, param_idx in enumerate(ts_orders):
            ...
            for uo in range(6):
                ...
                # 过滤掉不在当前窗口范围内的数据
                window_mask = (ts_time >= start_time) & (ts_time <= end_time)
                ts_time_windowed = ts_time[window_mask]
                ts_value_windowed = ts_value[window_mask]
                ...
                # tp_t.append(ts_time)
                tp_condition=np.full(ts_value_windowed.shape, ts_condition[uo])
                tp_n = tp_n + (list(ts_value_windowed))
                tp_t = tp_t + (list(ts_time_windowed))
                tp_con = tp_con+(list(tp_condition))

            # 使用zip将时间与值配对，并根据时间排序
            sorted_pairs = sorted(zip(tp_t, tp_n), key=lambda x: x[0])

            # 分解排序后的配对，恢复ts_time和ts_value为单独的列表
            sorted_ts_time, sorted_ts_value = zip(*sorted_pairs)

            # 使用zip将时间与值配对，并根据时间排序
            sorted_pairs = sorted(zip(tp_t, tp_con), key=lambda x: x[0])

            # 分解排序后的配对，恢复ts_time和ts_value为单独的列表
            sorted_ts_time, sorted_tp_con = zip(*sorted_pairs)
            ...
            if differ:  # using different colors and markers
                ...
                plt.plot(sorted_ts_time, sorted_ts_value, linestyle=linestyle, linewidth=linewidth, marker=marker, markersize=markersize,
                         color=param_color[0])
                # print()
            else:
                plt.plot(ts_time_windowed, ts_value_windowed, linestyle=linestyle, linewidth=linewidth, marker=marker,
        ...
        plt.rcParams['savefig.dpi'] = dpi  # default=100
        # plt.rcParams['figure.figsize'] = (img_width / dpi, img_height / dpi)
        plt.rcParams['figure.figsize'] = (cell_height / dpi, cell_width*8 / dpi)
        plt.rcParams['figure.frameon'] = False
        plt.figure(figsize=(cell_width/ dpi ,  cell_height*8/ dpi ),dpi=dpi)
        ...
        if differ:  # using different colors and markers
            ...
            #          marker=marker, markersize=markersize, color=param_color[ty])
            plt.margins(0, 0)
            plt.plot(sorted_ts_time, sorted_tp_con, linestyle=linestyle, linewidth=linewidth, marker=marker,
...
def construct_image(
    ...
    for idx, param in enumerate(ts_params):
        ts_color_mapping[param] = [plt_colors[idx * 6], plt_colors[idx * 6 + 1], plt_colors[idx * 6 + 2],
                                   plt_colors[idx * 6 + 3], plt_colors[idx * 6 + 4], plt_colors[idx * 6 + 5]]
    ...
    for split_idx in range(5):
        ...
        for group_idx in tqdm(range(num_groups)):
            ...
            for idx, p in enumerate(group):
                ...
                ts_condition.append(p['condition'])
            ...
            ts_condition = np.array(ts_condition)
            # draw the image for each p
            drawed_params = draw_image(pid, split_idx, ts_orders, ts_values, ts_times, ts_condition, ts_params,
                                       ts_scales, override,
